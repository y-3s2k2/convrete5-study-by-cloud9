...
<form method="post" action="<?php echo $view->action('save_urls'); ?>">
    ...
    <fieldset>
        ...
        <div class="form-group">
            <label class="control-label" for="canonical_url_alternative"><?php echo t('Alternative canonical URL') ?></label>
            <?php echo $form->text('canonical_url_alternative', $canonical_url_alternative, ['placeholder' => 'https://domain.com'])?>
        </div>
...
$(function () {
    var steps = [{
        ...
        content: <?php echo json_encode('<p><span class="h5">' . t('Alternative URL') . '</span><br/>' . t('Certain add-ons require a secure SSL URL. Enter that URL here.') . '</p>') ?>,
        highlightTarget: false,
        nextButton: true,
        ...
        content: <?php echo json_encode('<p><span class="h5">' . t('Alternative URL') . '</span><br/>' . t('Ensure that your site ONLY renders at the canonical URL or the alternative URL.') . '</p>') ?>,
        highlightTarget: false,
        nextButton: true,
