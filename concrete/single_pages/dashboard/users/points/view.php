...
<form class="form-inline" action="<?php echo $view->action('view')?>" method="get">
    ...
    <div class="ccm-pane-options">
        <div class="ccm-pane-options-permanent-search">
            <?php echo $form->label('uID', t('User'))?>
            <?php echo $form_user_selector->quickSelect('uID', $_GET['uID'], array('form-control'));?>
        </div>
    </div>

    <div class="clearfix" style="margin-top: 30px;">
        <input type="submit" value="<?php echo t('Search')?>" class="btn btn-primary pull-right" />
    </div>
</form>
<?php
if (count($entries) > 0) {
...
<div class="table-responsive">
	<table id="ccm-product-list" class="ccm-search-results-table compact-results">
    	<thead>
    		...
    		<th><span><?php echo t("Details")?></span></th>
    		<th></th>
    	</thead>
        ...
        ?><?php echo h($action->getUserPointActionName())?><?php
    }
            ?></td>
